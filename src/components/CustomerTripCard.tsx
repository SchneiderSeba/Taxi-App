  ...
  return (
    ...
    {lastRequest && (
            <section className="px-4 sm:px-6 pb-6 sm:pb-10 max-w-4xl mx-auto">
                <div className={`bg-white/25 dark:bg-gray-900 border rounded-xl sm:rounded-2xl shadow-md p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 ${
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                    <p className="text-xs sm:text-sm text-gray-700">Estado de tu solicitud</p>
                    ...
                    {lastRequest.status === 'pending' && (
                        <span className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 rounded-full bg-white/45 dark:bg-blue-900/40 text-blue-700 dark:text-blue-200 text-xs sm:text-sm font-semibold min-h-[44px]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                    <div>
                    <p className="uppercase text-[10px] sm:text-xs text-gray-800 tracking-wide">Origen</p>
                    <p className="font-semibold text-gray-900 dark:text-white mt-0.5">{lastRequest.pickup}</p>
                    </div>
                    <div>
                    <p className="uppercase text-[10px] sm:text-xs text-gray-800 tracking-wide">Destino</p>
                    <p className="font-semibold text-gray-900 dark:text-white mt-0.5">{lastRequest.destination}</p>
                    </div>
                    <div className="sm:col-span-2 lg:col-span-1">
                    <p className="uppercase text-[10px] sm:text-xs text-gray-800 tracking-wide">Horario</p>
